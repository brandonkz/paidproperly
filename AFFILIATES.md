# PaidProperly Affiliate Tracking

Last updated: 2026-02-07

## Priority Signups

### Affiliate Networks (Do These First)
| Network | Status | Commission | Sign Up Link | Notes |
|---------|--------|------------|--------------|-------|
| Impact Radius | ❌ REJECTED | N/A | N/A | Need more traffic/established site |
| PartnerStack | ⬜ TODO | Varies by program | https://partnerstack.com/partners | B2B SaaS programs |
| Awin | ⬜ TODO | Varies by program | https://www.awin.com/us/publishers | Absorbed ShareASale |
| CJ Affiliate | ⬜ TODO | Varies by program | https://www.cj.com/publisher | Large network |

### High-Value Direct Programs
| Platform | Status | Commission | Sign Up Link | Your Affiliate Link |
|----------|--------|------------|--------------|---------------------|
| Toptal | ⬜ TODO | $500-2,000/placement | https://www.toptal.com (login → referrals) | |
| Fiverr | ❌ REJECTED | N/A | N/A | Application rejected - too well known anyway |
| Respondent | ✅ APPROVED | $20-50/referral | In-app after login | https://app.respondent.io/r/brandonkatz-28807d50a084 |
| Preply | ⬜ TODO | 50% first payment | https://preply.com/en/affiliate | |
| Italki | ⬜ TODO | $10/new student | https://www.italki.com/affiliateprogram | |
| Hubstaff | ⬜ TODO | 30% recurring | https://hubstaff.com/affiliate | |
| Turing | ⬜ TODO | TBD | https://www.turing.com (check for referral) | |

---

## All Platforms - Affiliate Status

### ✅ Already Have Affiliate Links
| Platform | Current ref_url | Notes |
|----------|-----------------|-------|
| Somewhere | referral_referred_by=brandonkz@gmail.com | ✅ Active |
| Remote Rocketship | ?ref=brandon1d | ✅ Active |
| Respondent | /r/brandonkatz-28807d50a084 | ✅ Active - $20-50/referral |

### 🎯 High Priority (Likely Have Programs)
| Platform | Category | Status | Affiliate Link | Notes |
|----------|----------|--------|----------------|-------|
| Toptal | Tech & Dev | ⬜ TODO | | Top priority - high payouts |
| Turing | Tech & Dev | ⬜ TODO | | |
| Proxify | Tech & Dev | ⬜ TODO | | |
| Crossover | Competitive | ⬜ TODO | | |
| Fiverr | Marketplace | ❌ REJECTED | | Application rejected |
| Upwork | Marketplace | ⬜ TODO | | Not on site yet - consider adding |
| Preply | Teaching | ⬜ TODO | | |
| Italki | Teaching | ⬜ TODO | | |
| UserTesting | Testing | ⬜ TODO | | |
| Respondent | Testing | ⬜ TODO | | |
| Hubstaff Talent | Freelance | 🗑️ REMOVED | | Removed Feb 11 - poor website quality |
| Zapier | SaaS | ⬜ TODO | | Check via Impact |
| Shopify | SaaS | ⬜ TODO | | Check via Impact |

### 📋 Medium Priority (Check for Programs)
| Platform | Category | Status | Notes |
|----------|----------|--------|-------|
| Braintrust | Tech | ⬜ | User-owned, may not have affiliate |
| Arc.dev | Tech | ⬜ | |
| Gun.io | Tech | ⬜ | |
| Lemon.io | Tech | ⬜ | |
| Contra | Freelance | ⬜ | |
| Malt | Freelance | ⬜ | |
| Codementor | Teaching | ⬜ | |
| Belay | VA | ⬜ | |
| Time Etc | VA | ⬜ | |
| Outlier | AI Training | ⬜ | |
| Scale AI | AI Training | ⬜ | |
| Rev | Transcription | ⬜ | |

### 🏢 Job Boards & Big Companies (Usually No Affiliate)
| Platform | Notes |
|----------|-------|
| We Work Remotely | Job board - unlikely |
| Dynamite Jobs | Job board - unlikely |
| Support Driven | Job board - unlikely |
| Kraken | Crypto exchange - check for referral |
| Binance | Has referral program! Check |
| Coinbase | Has referral program! Check |
| Zapier | Check via Impact/PartnerStack |
| Automattic | Unlikely |
| Doist | Unlikely |

### 🎧 Customer Support Companies (Usually No Affiliate)
| Platform | Notes |
|----------|-------|
| Teleperformance | Employment - no affiliate |
| Concentrix | Employment - no affiliate |
| TTEC | Employment - no affiliate |
| Foundever | Employment - no affiliate |
| ModSquad | Employment - no affiliate |
| SupportNinja | Employment - no affiliate |

---

## Crypto Platforms (Check Referrals!)
These often have generous referral programs:

| Platform | Status | Sign Up | Notes |
|----------|--------|---------|-------|
| Binance | ⬜ TODO | binance.com/en/activity/referral | Up to 40% commission |
| Coinbase | ⬜ TODO | coinbase.com/affiliates | |
| Kraken | ⬜ TODO | kraken.com/features/affiliate | |

---

## Workflow

1. **Sign up for networks first** (Impact, PartnerStack, Awin)
2. **Apply to high-value direct programs** (Toptal, Upwork, etc.)
3. **Update this doc** with status and your affiliate links
4. **Tell Milan** when you have links — I'll update platforms.json and regenerate /go/ pages

---

## Status Key
- ⬜ TODO - Not started
- 🟡 APPLIED - Waiting for approval
- ✅ APPROVED - Have affiliate link
- ❌ REJECTED - No program or rejected
- ➖ N/A - No affiliate program exists
